...
final class SharedPreferencesService implements LocalService {
  ...
  Future<Either<LocalException, T?>> get<T>(String key) async {
    try {
      ...
      log("Error loading local String or Bool", error: e, stackTrace: st);
      return Left(const LocalException(message: 'Error getting value'));
    }
  }
  ...
  Future<Either<LocalException, void>> set<T>(String key, T data) async {
    try {
      ...
      log("Error saving local String or Bool", error: error, stackTrace: st);
      return Left(const LocalException(message: 'Error setting value'));
    }
  }
}
